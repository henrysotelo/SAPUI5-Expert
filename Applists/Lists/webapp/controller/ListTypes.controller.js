sap.ui.define([
    ...
    function (Controller, JSONModel) {
        ...
        return Controller.extend("logaligroup.Lists.controller.ListTypes", {
            onInit: function () {
                var oJSONMondel = new JSONModel();
                oJSONMondel.loadData("./localService/ListData.json");
                this.getView().setModel(oJSONMondel);
            },
            ...
            onDeleteRow: function(oEvent){
                ...
                var contex = selectedRow.getBindingContext();
                var splitPath = contex.getPath().split("/");
                var indexSelectedRow = splitPath[splitPath.length-1];
                var model = this.getView().getModel();
                var product = model.getProperty("/Products");
                product.splice(indexSelectedRow, 1);
                model.refresh();
                sap.m.MessageToast.show("Elimindado");
            }
